...
pipeline {
    agent {
        docker {
            ...
            args '-u root:root'  // so npm install doesn't hit permissions
        }
    }
    ...
    stages {
        ...
        stage('Run Playwright Tests on All Browsers') {
            parallel {
                stage('Chromium') {
                    steps {
                        sh """
                          export TEST_ENV=${params.TEST_ENV}
                          xvfb-run -a npx playwright test --project=chromium --reporter=junit,html --output=results/chromium
                        """
                    }
                }
                stage('Firefox') {
                    steps {
                        sh """
                          export TEST_ENV=${params.TEST_ENV}
                          xvfb-run -a npx playwright test --project=firefox --reporter=junit,html --output=results/firefox
                        """
                    }
                }
                stage('WebKit') {
                    steps {
                        sh """
                          export TEST_ENV=${params.TEST_ENV}
                          xvfb-run -a npx playwright test --project=webkit --reporter=junit,html --output=results/webkit
                        """
                    }
                }
            }
