...
import { ResellerUserFormPage } from '../../resellerUserFormPage';
...
test.describe('Login Success Tests', () => {
 ...
 await page.waitForLoadState('networkidle');
  ...
  if (await userRow.count() > 0) {
    ...
    // Click the "View" link in that row
...
// Validate fields
  ...
  //const usersSection = page.locator('div:has-text("Users Create Reseller User")');
  //await expect(usersSection).toBeVisible();

  await expect(page.locator('h4:has-text("Users")')).toHaveText('Users');

  const createUserLink = page.locator('a:has-text("Create Reseller User")');
  await expect(createUserLink).toBeEnabled();
  await createUserLink.click();

  const modal = page.locator('.modal-content');
await expect(modal.locator('h5#myModal-label')).toHaveText('Create Reseller');

const resellerUserForm = new ResellerUserFormPage(page);
...
await expect(page.getByRole('alert')).toHaveText(/Reseller User successfully Created/i);
//Load the page 
await page.reload()

const row = page.locator('table tr', { hasText: 'jdoe@example.com' });
await expect(row.locator('td').nth(0)).toHaveText('John Doe');
await expect(row.locator('td').nth(2)).toHaveText('jdoe@example.com');
