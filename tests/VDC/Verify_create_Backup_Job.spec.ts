...
test('Should be able to successfully Clone Virtual Machine - Service Provider @dev @staging @preprod', async ({ page }) => {
  ...
  const modal = new CreateBackupJobModal(page);

  await modal.open();

  await modal.fillBasicInfoAndSchedule({
    name: 'Auto-Backup-Job-1',
    enabled: true,
    storage: '4',        // Backupstorage option
    mode: 'snapshot',    // snapshot / suspend / stop
    schedule: 'hourly'    // Every Day
  });

  // Step 2: select a VM
  await modal.selectVMs(['AutoVM-001']); // can pass multiple names

  await modal.setRetention({ hourly: 1 }); // Only one value is fine
  await modal.toggleKeepAllBackups(false);

 await modal.saveJob();


  await expect(page.locator('.alert-success')).toContainText('Backup Job has been successfully created.');
