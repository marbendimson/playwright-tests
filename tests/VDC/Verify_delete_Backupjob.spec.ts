...
import { BackupJobActions } from '../../backupjobmodal';
...
test('Should be able to successfully Clone Virtual Machine - Service Provider @dev @staging @preprod', async ({ page }) => {
  ...
  const backupJob = new BackupJobActions(page);

  await backupJob.openDeleteModal();
  await backupJob.verifyDeleteModal();
  await backupJob.cancelDelete();

  await backupJob.openDeleteModal();
  await backupJob.verifyDeleteModal();
  await backupJob.confirmDelete();
