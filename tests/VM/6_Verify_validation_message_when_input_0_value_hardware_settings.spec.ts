...
const nextStepBtn = page.locator('button.btn.btn-success.nexttab[data-nexttab="steparrow-storage-configuration-tab"]');

  // 1. Verify button visible
  await expect(nextStepBtn).toBeVisible({ timeout: 5000 });

  // 2. Verify button enabled (clickable)
  ...
  const messages = [
    'Sockets must be no less than 1.',
    'Cores must be no less than 1.',
    'Memory must be no less than 1.' 
    
  ];

  for (const msg of messages) {
    const feedback = page.locator('.invalid-feedback', { hasText: msg });
