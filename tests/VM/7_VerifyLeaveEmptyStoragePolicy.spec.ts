...
test('Create VM - All in One Flow', async ({ page }) => {
  ...
  // Step 4: Storage
  const storageConfigured = await vmPage.configureStorage(page, 0 );
  if (!storageConfigured) return;

  const nextStepBtn = page.locator('button.btn.btn-success.nexttab[data-nexttab="steparrow-network-configuration-tab"]');
  ...
  await expect(
    page.locator('div.invalid-feedback', { hasText: 'Disk size must be greater than 0' })
