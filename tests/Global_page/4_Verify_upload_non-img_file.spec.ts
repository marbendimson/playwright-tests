...
test.describe('Verify Upload non-img file ', () => {
  test('Should display prompt message successfully ', async ({ page }) => {
...
await brandingTab.click();
      ...
      const logoPath = path.join(__dirname, '../fixtures/Virtual_Data_Center_Usage_Report_2025-05-05_02-34-52.xls');
  await page.locator('#profile-img-file-input-logo').setInputFiles(logoPath);

 const errorPopup = page.locator('.swal2-popup.swal2-modal.swal2-icon-error');
  await expect(errorPopup).toBeVisible();
  await expect(page.locator('#swal2-title')).toHaveText('Invalid File Type');
   const errorMessage = page.locator('#swal2-html-container');
  await expect(errorMessage).toBeVisible();
  await expect(errorMessage).toHaveText('Please select JPEG, PNG, or GIF files only.');
